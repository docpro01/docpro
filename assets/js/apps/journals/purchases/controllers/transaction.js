angular.module('journals')
		...
		//Document
			...
			$scope.document_name = 'Purchases Journal';
			
		//Business Partner
			$scope.selected_bp = {};
			$scope.business_partner_array = [];
			$http.get(window.location.origin+'/journals/purchases/transaction/bp/get').then(function success(response){
		...
		//MODE OF PAYMENT
			...
			$http.get(window.location.origin+'/journals/purchases/transaction/mop/get').then(function success(response){
				$.each(response.data, function(index, data){
