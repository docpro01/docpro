angular.module('journals')
		...
		//Document
			...
			$scope.$watch('payment_amount_paid', function(){
				$scope.amount_in_words = ($scope.payment_amount_paid !== '') ? toTitleCase(toWords(parseFloat($scope.payment_amount_paid))) : '';
			});
			...
			$scope.document_name = 'Disbursements Journal';
			
		//Business Partner
			$scope.selected_bp = {};
			$scope.business_partner_array = [];
			$http.get(window.location.origin+'/journals/disbursements/transaction/bp/get').then(function success(response){
		...
		//MODE OF PAYMENT
			...
			$http.get(window.location.origin+'/journals/disbursements/transaction/mop/get').then(function success(response){
				$.each(response.data, function(index, data){
