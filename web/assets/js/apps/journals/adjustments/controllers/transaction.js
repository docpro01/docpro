angular.module('journals')
		...
		//Document
			...
			$scope.document_name = 'Adjustments Journal';
			
		//Business Partner
			$scope.selected_bp = {};
			$scope.business_partner_array = [];
			$http.get(window.location.origin+'/journals/adjustments/transaction/bp/get').then(function success(response){
		...
		//MODE OF PAYMENT
			...
			$http.get(window.location.origin+'/journals/adjustments/transaction/mop/get').then(function success(response){
				$.each(response.data, function(index, data){
