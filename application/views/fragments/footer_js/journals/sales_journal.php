...
<script>
	var summary_table = $('#sales-journal-summary-table').DataTable({
							ajax: window.location.origin+'/journals/sales/journals_summary/get',
							columns: [
										{'data': 'co_trans_id'},
										{
	...
	var bp_summary_table = $('#sales-bp-table').DataTable({
								ajax: window.location.origin+'/journals/sales/bp/get',
								columns: [
											{'data': 'co_bp_class'},
											{'data': 'co_bp_code'},
											{'data': 'bp_name'},
	...
	var bp_transaction_details_table = $('#sales-transaction-details-table').DataTable({
							ajax: window.location.origin+'/journals/sales/bp_transaction/get?bp_id=0',
							columns: [
										{'data': 'co_trans_id'},
										{
											mData: function(rowData, settings, fullData){
...
<script>
	$('#sales-journal-summary-table_wrapper tbody').on('click', 'tr', function(){
	...
	$('#sales-bp-table_wrapper tbody').on('click', 'tr', function(){
		var row_data = bp_summary_table.row(this).data();
		bp_transaction_details_table.ajax.url(window.location.origin+'/journals/sales/bp_transaction/get?bp_id='+row_data.co_bp_id).load();
		$(this).closest('table').find('tr').css('background-color', '#FFF');
		$(this).closest('table').find('tr').css('color', 'rgba(0,0,0,.87)');
		$(this).css('background-color', 'rgb(133, 150, 214)');
		$(this).css('color', '#FFF');
	...
	$('#sales-transaction-details-table_wrapper tbody').on('click', 'tr', function(){
...
<script>
	...
	// 	var inputs = $('#new-transactions').find('input');
	// 	$.each(inputs, function(index, data){
	// 		if($(data).attr('type') !== 'hidden'){
	// 			$(data).focusout(function(){
	// 				$(data).removeClass('has-focus');
	// 				$(data).validate(data, {required: true});
	// 				setTimeout(function(){
	// 					if($(data).closest('.ui-select-container').hasClass('ng-valid')){
	// 						$(data).closest('.form-group').find('.error-msg').remove();
	// 					}
	// 				}, 1000);
					
	// 			});
	// 			$(data).focusin(function(){
	// 				$(data).addClass('has-focus');
	// 			});
	// 		}
	// 	});
	// });
	...
	$(document).ready(function(){
		var inputs = [
						...
						// },
						// {
						// 	name: '.selectize-input input',
						// 	rules: {required: true}
						// },
						// {
						// 	name: 'particulars_period',
						// 	rules: {required: true}
						// },
						// {
						...
						// },
						// {
						// 	name: 'amounts_tax_withheld',
						// 	rules: {required: true}
						// },
						// {
						// 	name: 'amounts_deductions',
						// 	rules: {required: true}
						// },
					];

		$.each(inputs, function(index, data){
			$('input[name='+data.name+']').focusout(function(){
				$('input[name='+data.name+']').validate($('input[name='+data.name+']'), data.rules);
			});
			$('input[name='+data.name+']').focusin(function(){
				$('input[name='+data.name+']').addClass('has-focus');
			});
